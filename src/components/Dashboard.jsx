...
export default function Dashboard() {
  ...
  const handleUpdateDemoLayout = (id, newX, newY) => { setDemoLayout(prev => prev.map(item => item.id === id ? { ...item, x: newX, y: newY } : item)); };
  ...
  return (
    <div className={`h-screen w-screen bg-tech-black text-white flex overflow-hidden font-sans ${isResizing ? 'cursor-col-resize select-none' : ''}`}>
      ...
      <div className={`flex-1 relative bg-gradient-to-b from-tech-black to-tech-gray min-w-0 flex flex-col ${isResizing ? 'pointer-events-none' : ''}`} onDrop={handleDrop} onDragOver={e => e.preventDefault()}>
        ...
        <div className="flex-1 relative overflow-hidden">{viewMode === 'simulation' ? <Scene bubbles={bubbles} activeId={activeFeature} setActiveId={setActiveFeature} totalUsers={totalUsers} /> : <FullCodeEditor isConnected={isConnected} onConnect={handleConnect} />}</div>
      </div>
