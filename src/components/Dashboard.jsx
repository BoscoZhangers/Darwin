...
const IframeRenderer = ({ code, onUpdateCode, handleUpdateLayout, mode, onExtractStart }) => {
  ...
  const srcDoc = `<!DOCTYPE html><html><head><meta charset="utf-8" /><script src="https://unpkg.com/react@18/umd/react.development.js"></script><script src="https://unpkg.com/react-dom@18/umd/react-dom.development.js"></script><script src="https://unpkg.com/@babel/standalone/babel.min.js"></script><style>body { margin: 0; overflow: hidden; background: #fff; user-select: none; } .mode-edit .darwin-draggable { cursor: move; } .mode-edit .darwin-draggable:hover { outline: 2px solid #00f3ff; } .mode-live .darwin-draggable { cursor: grab; } .mode-live .darwin-draggable:hover { outline: 2px dashed #bc13fe; cursor: alias; }</style></head><body class="mode-${mode}"><div id="root"></div><script type="text/babel">const { useState, useEffect, useRef } = React; const DarwinTracker = () => null; const InteractiveElement = ({ _tag: Tag, _darwinIndex, children, style, ...props }) => { const isAbsolute = style && style.position === 'absolute'; const hasId = props['data-darwin-id'] || props.id; const canInteract = isAbsolute || ('${mode}' === 'live' && hasId); if (!canInteract) return <Tag style={style} {...props}>{children}</Tag>; const [pos, setPos] = useState({ x: parseInt(style?.left || 0), y: parseInt(style?.top || 0) }); const [isDragging, setIsDragging] = useState(false); const dragOffset = useRef({ x: 0, y: 0 }); const handleMouseDown = (e) => { e.stopPropagation(); e.preventDefault(); if ('${mode}' === 'live') { const rect = e.target.getBoundingClientRect(); const computed = window.getComputedStyle(e.target); const meta = { width: Math.round(rect.width), height: Math.round(rect.height), x: Math.round(rect.x), y: Math.round(rect.y), bgColor: computed.backgroundColor, color: computed.color, type: Tag }; window.parent.postMessage({ type: 'EXTRACT_COMPONENT', tag: props['data-darwin-id'] || props.id || Tag, id: _darwinIndex, clientX: e.clientX, clientY: e.clientY, meta: meta }, '*'); return; } setIsDragging(true); dragOffset.current = { x: e.clientX - pos.x, y: e.clientY - pos.y }; }; useEffect(() => { if (!isDragging || '${mode}' === 'live') return; const handleMove = (e) => { let newX = e.clientX - dragOffset.current.x; let newY = e.clientY - dragOffset.current.y; if (newX < 0) newX = 0; if (newY < 0) newY = 0; setPos({ x: newX, y: newY }); }; const handleUp = () => { setIsDragging(false); window.parent.postMessage({ type: 'UPDATE_POS', index: _darwinIndex, x: pos.x, y: pos.y }, '*'); }; window.addEventListener('mousemove', handleMove); window.addEventListener('mouseup', handleUp); return () => { window.removeEventListener('mousemove', handleMove); window.removeEventListener('mouseup', handleUp); }; }, [isDragging, pos]); return <Tag style={{ ...style, left: pos.x, top: pos.y }} className="darwin-draggable" onMouseDown={handleMouseDown} {...props}>{children}</Tag>; }; try { ${prepareTransformedCode()} const root = ReactDOM.createRoot(document.getElementById('root')); root.render(<App />); } catch (err) { document.body.innerHTML = '<div style="color:red; padding:20px">Preview Error: ' + err.message + '</div>'; } </script></body></html>`;
...
export default function Dashboard({ user, token, repo, onBack }) {
  ...
  useEffect(() => { if (!extractedGhost) return; const handleGlobalMove = (e) => { setExtractedGhost(prev => ({ ...prev, x: e.clientX, y: e.clientY })); }; const handleGlobalUp = (e) => { const sceneWidth = window.innerWidth - rightPanelWidth; if (e.clientX < sceneWidth) { const nextColor = NEON_PALETTE[bubbles.length % NEON_PALETTE.length]; const normalizedX = ((e.clientX / sceneWidth) - 0.5) * 20; const normalizedZ = ((e.clientY / window.innerHeight) - 0.5) * 20; setBubbles(prev => { if(prev.find(b => b.id === extractedGhost.id)) return prev; return [...prev, { id: extractedGhost.id, label: extractedGhost.tag, count: 0, visible: true, color: nextColor, position: [normalizedX, 0, normalizedZ], meta: extractedGhost.meta || {} }]; }); setAiLog(prev => [...prev, { role: 'success', text: `Added tracking for ${extractedGhost.tag}` }]); setActivePanel('properties'); } setExtractedGhost(null); }; window.addEventListener('mousemove', handleGlobalMove); window.addEventListener('mouseup', handleGlobalUp); return () => { window.removeEventListener('mousemove', handleGlobalMove); window.removeEventListener('mouseup', handleGlobalUp); }; }, [extractedGhost, rightPanelWidth, bubbles]);
      ...
      <div style={{ width: rightPanelWidth }} className="flex flex-col bg-[#111] shrink-0 border-l border-white/5 relative">
         ...
         <div className="h-1/2">
           <div className="flex flex-col h-full bg-[#1e1e1e]">
              <div className="h-10 flex items-center justify-between px-4 bg-[#252526] border-b border-black/20"><span className="text-[10px] font-bold uppercase text-gray-400 flex items-center gap-2"><Globe size={12}/> Live Preview</span><div className={`text-[10px] px-2 rounded flex items-center gap-1 ${demoMode ? 'bg-blue-500/20 text-blue-400' : 'bg-purple-500/20 text-purple-400'}`}>{demoMode ? 'EDIT MODE' : 'EXTRACT MODE'}</div></div>
         ...
         <div className="h-1/2 border-t border-white/10 flex flex-col bg-black/40">
            <div className="flex border-b border-white/10">
               <button onClick={() => setActivePanel('logs')} className={`px-4 py-2 text-[10px] font-bold uppercase flex items-center gap-2 ${activePanel === 'logs' ? 'bg-white/10 text-yellow-500 border-b-2 border-yellow-500' : 'text-gray-500 hover:text-white'}`}><Zap size={12} /> System Logs</button>
               <button onClick={() => setActivePanel('properties')} className={`px-4 py-2 text-[10px] font-bold uppercase flex items-center gap-2 ${activePanel === 'properties' ? 'bg-white/10 text-green-400 border-b-2 border-green-400' : 'text-gray-500 hover:text-white'}`}><Shapes size={12} /> Properties</button>
            </div>
            <div className="flex-1 overflow-y-auto font-mono text-[10px] p-0">
               {activePanel === 'logs' ? ( <div className="p-4 space-y-2">{aiLog.map((log, i) => <div key={i} className={`p-2 rounded border-l-2 ${log.role === 'success' ? 'border-green-500 bg-green-500/5 text-green-400' : 'border-gray-700 bg-white/5 text-gray-500'}`}>{log.text}</div>)}</div> ) : ( <div className="p-2 space-y-1">{bubbles.length === 0 ? (<div className="text-gray-600 text-center py-8 italic">No active trackers. Drag elements from the preview here.</div>) : (bubbles.map((b) => (<div key={b.id} className={`bg-[#222] rounded overflow-hidden border border-transparent hover:border-white/10 transition-colors ${!b.visible ? 'opacity-50' : ''}`}><div className="flex items-center justify-between p-2 hover:bg-[#333]"><div className="flex items-center gap-2 cursor-pointer flex-1" onClick={() => toggleExpand(b.id)}>{expandedProperties.has(b.id) ? <ChevronDown size={14} className="text-gray-400" /> : <ChevronRight size={14} className="text-gray-400" />}<div className={`w-2 h-2 rounded-full ${b.visible ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'bg-gray-600'}`} style={{ backgroundColor: b.color || 'gray' }} /><span className="text-gray-300 font-bold text-xs">{b.label || 'Component'}</span><span className="text-gray-500 text-[8px] uppercase tracking-wider ml-1">#{b.id}</span></div><div className="flex items-center gap-1"><button onClick={(e) => { e.stopPropagation(); toggleVisibility(b.id); }} className="text-gray-500 hover:text-white p-1 rounded hover:bg-white/10">{b.visible ? <Eye size={12} /> : <EyeOff size={12} />}</button><button onClick={(e) => { e.stopPropagation(); handleDeleteBubble(b.id); }} className="text-gray-500 hover:text-red-500 p-1 ml-2 transition-colors" title="Delete Tracker"><Trash2 size={12} /></button></div></div>{expandedProperties.has(b.id) && (<div className="bg-black/40 p-2 text-[9px] text-gray-400 space-y-1 border-t border-white/5"><div className="flex items-center justify-between"><div className="flex items-center gap-1"><MousePointerClick size={10}/> Interactions</div> <span className="text-green-400 font-mono">{b.count}</span></div><div className="flex items-center justify-between"><div className="flex items-center gap-1"><Code size={10}/> Type</div> <span className="text-white font-mono">{b.meta?.type || 'Unknown'}</span></div><div className="flex items-center justify-between"><div className="flex items-center gap-1"><Move size={10}/> Dimensions</div> <span>{b.meta?.width}x{b.meta?.height}</span></div><div className="flex items-center justify-between"><div className="flex items-center gap-1"><MapPin size={10}/> Position</div> <span>{b.meta?.x}, {b.meta?.y}</span></div><div className="flex items-center justify-between"><div className="flex items-center gap-1"><Palette size={10}/> Color</div><div className="flex items-center gap-2"><input type="color" value={colorToHex(b.meta?.bgColor)} onChange={(e) => handleStyleChange(b.id, 'bgColor', e.target.value)} className="w-8 h-8 p-0 border border-white/10 rounded" /><div className="w-3 h-3 rounded-full border border-white/20" style={{backgroundColor: b.meta?.bgColor}}></div><span className="font-mono ml-2">{b.meta?.color}</span></div></div><div className="flex items-center justify-between mt-1"><div className="flex items-center gap-1"><span className="text-[10px]">Font-size</span></div><div className="flex items-center gap-2"><input type="number" value={b.meta?.fontSize ? parseInt(b.meta.fontSize) : ''} onChange={(e) => handleStyleChange(b.id, 'fontSize', `${e.target.value}px`)} className="w-20 p-1 rounded border border-white/10 text-black text-[10px]" /></div></div></div>)}</div>)))}</div>)}
            </div>
         </div>
      </div>
    </div>
