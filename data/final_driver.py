...
def design_agent(number_iter,list_of_classes):
    ...
    '''
    client = genai.Client(api_key = 'AIzaSyDGUiL1fNTRpIG28S7c3XODNqYsOWVy1-s')
...
def click_agent(output_len, html_files,list_of_classes):
    ...
    For x and y, have them represent the upper left corner of the div. Note that the list of ids you can change is %s. Ensure that all ids
    get some coverage, but keep the distribution of clicks realistic. Only click on the ids
    ...
    for attempt in range(max_retries):
        try:
            ...
            if "503" in str(e) or "overloaded" in str(e).lower():
                ...
                # If it's a different error (like 400 Bad Request), crash immediately so you can fix it
                raise e
    
    raise Exception("Failed to get response after multiple retries due to server overload.")


def nn_model(df, NUM_DIVS = 5, EMBEDDING_DIM = 256, INNER_LAYER_SIZE = 128, TRAIN_EPOCHES = 3000, predict_x = None, predict_y = None, predict_id = None, predict_other = None, predict_ref = None):
...
def __main__():
    ...
    # Run Agents
    html_files = design_agent(10,list_of_classes)
    click_data = click_agent(50, html_files,list_of_classes)
    ...
    grouped_df = final_df.groupby(final_df.columns[:-1].tolist()).agg({'hits': 'sum'}).reset_index()
    
    nn_model(grouped_df)
    grouped_df.to_csv('pain.csv')
    print('---------------------------------Done :D---------------------------------')
